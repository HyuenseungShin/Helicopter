...
public class Game {
    ...
    private ArrayList<Animation> coinList;
    private BufferedImage explosionAnimImg;
    private BufferedImage coinAnimImg;
    ...
    private BufferedImage skyColorImg1;
    private BufferedImage skyColorImg2;
    private BufferedImage skyColorImg3;
    private BufferedImage skyColorImg4;
    ...
    public BufferedImage getSkyColor() {
    	   if(stageNum == (1+count))
       	    skyColorImg = skyColorImg1;
       	else if(stageNum == (4+count))
       		skyColorImg = skyColorImg2;
       	else if (stageNum ==(7+count))
       	    skyColorImg = skyColorImg3;
       	else if (stageNum ==(10+count))
       	    skyColorImg = skyColorImg4;
           if(stageNum==10)
          	    count+=12;
     if(stageNum==10)
          	 count+=12;
   ...
   /**
    ...
    private void Initialize()
    {
        ...
        coinList = new ArrayList<Animation>();
    ...
    private void LoadContent()
    {
         ...
         String weaponPackage = "/helicopterbattle/resources/images/weapon";
         String explosionPackage = "/helicopterbattle/resources/images/explosion";

         skyColorImg1  =  this.drawBufferImage( backgroundPackage + "/sky_color.jpg");
         skyColorImg2  = this.drawBufferImage( backgroundPackage + "/sky_color2.jpg");
         skyColorImg3  =  this.drawBufferImage( backgroundPackage + "/sky_color3.jpg");
         skyColorImg4  =  this.drawBufferImage( backgroundPackage + "/sky_color4.jpg");
         ...
         mountainsImg =  this.drawBufferImage( backgroundPackage + "/mountains.png");
         groundImg =  this.drawBufferImage( backgroundPackage + "/ground.png");


         // Load images for enemy helicopter
         EnemyHelicopter.helicopterBodyImg =  this.drawBufferImage( helicopterPackage + "/2_helicopter_body.png");
         EnemyHelicopter.helicopterFrontPropellerAnimImg =  this.drawBufferImage( helicopterPackage + "/2_front_propeller_anim.png");
         EnemyHelicopter.helicopterRearPropellerAnimImg =  this.drawBufferImage( helicopterPackage + "/2_rear_propeller_anim.png");

         // Image of mouse cursor.
         mouseCursorImg =  this.drawBufferImage( helicopterPackage + "/mouse_cursor.png");

         // Images of rocket and its smoke.
         Rocket.rocketImg =  this.drawBufferImage( weaponPackage + "/rocket.png");
         RocketSmoke.smokeImg =  this.drawBufferImage( weaponPackage +"/rocket_smoke.png");

         // Helicopter machine gun bullet.
         Bullet.bulletImg =  this.drawBufferImage( weaponPackage + "/bullet.png");
         EnemyBullet.bulletImg =  this.drawBufferImage( weaponPackage + "/bullet.png");

         // Imege of explosion animation.
         explosionAnimImg =  this.drawBufferImage( explosionPackage + "/explosion_anim.png");
         coinAnimImg =  this.drawBufferImage( explosionPackage + "/COIN2.png");
    ...
    public void UpdateGame(long gameTime, Point mousePosition)
    {
    	...
    	if(isPlayerAlive()&&destroyedEnemies==goalNum) {
         	Framework.gameState = Framework.GameState.STAGE_CLEAR;
         	goalNum+=2;
         	return;
         }
        /* Player */
    ...
    public void Draw(Graphics2D g2d, Point mousePosition, long gameTime)
    {
        ...
        g2d.drawImage(getSkyColor(), 0, 0, Framework.frameWidth, Framework.frameHeight, null);
        ...
        g2d.drawString("COINS: "      + player.coins, 10, 121);
        g2d.drawString("PLAYER HEALTH: "      + player.health, 10, 141);
    ...
    public void DrawStatistic(Graphics2D g2d, long gameTime){
        ...
        g2d.drawString("Coins: "             + player.coins,           Framework.frameWidth/2 - 65, Framework.frameHeight/3 + 190);
        g2d.setFont(font);
    ...
    private void createEnemyHelicopter(long gameTime)
    {
        if(gameTime - EnemyHelicopter.timeOfLastCreatedEnemy >= EnemyHelicopter.timeBetweenNewEnemies)
        {
            ...
            if(yCoordinate % 3 == 0) {
                position = 0;
            } else if(yCoordinate % 3 == 1)  {
                position = 1;
            } else {
                position = 2;
            }
            
            eh.BulletInitialize(xCoordinate, yCoordinate, position);
    ...
    private void updateEnemies()
    {
        for(int i = 0; i < enemyHelicopterList.size(); i++)
        {
            ...
            eh.Update();
            if(eh.isShootBullet()) {
                EnemyBullet enemyBullet = new EnemyBullet(eh.xCoordinate, eh.yCoordinate);
            ...
            if(playerRectangel.intersects(enemyRectangel)){
                ...
                for(int exNum = 0; exNum < 3; exNum++){
                    Animation expAnim = new Animation(explosionAnimImg, 134, 134, 12, 30, false, eh.xCoordinate + exNum*60,
                    		                          eh.yCoordinate - random.nextInt(100), exNum * 200 +random.nextInt(100));
                    explosionsList.add(expAnim);
            ...
            if(eh.health <= 0){
            	...
            	Music deadSound = new Music("deadsound.mp3" , false);
            	deadSound.start();
                ...
                // Substring 1/3 explosion image height (explosionAnimImg.getHeight()/3) so that explosion is drawn 
                //  more at the center of the helicopter.
                explosionsList.add(expAnim);
                Animation plusCoin = new Animation(coinAnimImg,  36, 36, 10, 45, false, eh.xCoordinate, 
                		eh.yCoordinate - coinAnimImg.getHeight()/3, 0);
                coinList.add(plusCoin);
    ...
    private void updateEnemyBullets()
    {
        for(int i = 0; i < enemyBulletsList.size(); i++)
        {
            ...
            Rectangle bulletRectangle = new Rectangle((int)enemyBullet.xCoordinate, (int)enemyBullet.yCoordinate, Bullet.bulletImg.getWidth(), Bullet.bulletImg.getHeight());
            Rectangle playerRectangel = new Rectangle(player.xCoordinate, player.yCoordinate, EnemyHelicopter.helicopterBodyImg.getWidth(), EnemyHelicopter.helicopterBodyImg.getHeight());
